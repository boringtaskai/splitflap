...
// double sided kyk kertas mau di print tapi di balik, jadi posisi baliknya 
...
// ini mirip kyk double-sided hanya saja layoutnya tidak dibalik, hanya 
// per individunya saja yang dibalik
MODE_FRONT_BACK = 1;
...
// To print of ID Card PVC 0.76mm with double-side
MODE_FULL_FONT_PVC = 4;


layout_mode = MODE_FULL_FONT_PVC;
...
num_character = len(character_list);
cur_character = 0;
...
// untuk menghitung posisi X dan Y dari flap,
// jika MODE_SIDE_BY_SIDE, berarti posisi X nya harus digeser 2 dari posisi flap
// karena yang seharusnya dibaliknya, jadi di sampingnya
...
//    y_pos = (flap_height + spacing_y) * row;
...
// untuk menghitung posisi X dan Y dari flap
// jika ada parameter, dimulai dari row ke berapa, 
// dan juga jika di stop pada row keberapa
module flap_pos(i) {
    col = i % cols;
    row = floor(i / cols);
    offsetted_row = row - start_row;
    
    if (layout_mode == MODE_FULL_FONT_PVC) {
        remaining = num_character - i;
        last_remaining = num_character % (cols * 2);
        last_row = (num_character - last_remaining) / cols;
        row = (row > last_row) ? last_row : row;
        offsetted_row = row - start_row;
        
        if (remaining < last_remaining) {
            is_godown =  remaining % 2;
            col = i % (cols * 2);
            backward_factor = (col > 0) ? floor((col -1) / 2) : 0;

//            echo(num_character=num_character);
//            echo(i=i);
//            echo(row=row);
//            echo(col=col);
//            echo(remaining=remaining);
//            echo(last_remaining=last_remaining);
//            echo(backward_factor=backward_factor);
//            echo(is_godown=is_godown);
            
            if (is_godown == 1) {
                x_pos = (flap_width + spacing_x) * (backward_factor);
                y_pos = (flap_height + spacing_y) * (offsetted_row + 1);
                translate([x_pos, -y_pos, 0]) {
                    children();
                }
            }
            else {
                x_pos = (flap_width + spacing_x) * (backward_factor+1);
                y_pos = (flap_height + spacing_y) * (offsetted_row);
                translate([x_pos, -y_pos, 0]) {
                    children();
                }
            }
    ...
    else if (offsetted_row >= 0 && offsetted_row < row_count) {
        flap_transform(offsetted_row, col) {
            children();
        }
    }   
...
module configured_flap(index, flap, front_letter, back_letter) {
    ...
    if (layout_mode == MODE_FULL_FONT_PVC) {
        remaining = num_character - index;
        last_remaining = num_character % (cols * 2);
        last_row = (num_character - last_remaining) / cols;
        row = (floor(index / cols) > last_row) ? last_row : floor(index / cols);
        is_rotate =  row % 2;
       
        if (is_rotate == 1) {
            rotate([0, 0, 180]) {
               translate([-flap_width, -flap_height - (2 * flap_gap) + 0.6, 0]) {
