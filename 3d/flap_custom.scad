...
function is_emoji_char(c) =
    let(u = ord(c))
    (
        (u >= 0x1F600 && u <= 0x1F64F) ||  // Emoticons
        (u >= 0x1F300 && u <= 0x1F5FF) ||  // Misc Symbols & Pictographs
        (u >= 0x1F680 && u <= 0x1F6FF) ||  // Transport / Map
        (u >= 0x1F900 && u <= 0x1F9FF) ||  // Supplemental Symbols
        (u >= 0x2600  && u <= 0x26FF ) ||  // Misc symbols
        (u >= 0x2700  && u <= 0x27BF )     // Dingbats
    );

module flap_2d(cut_tabs = false) {
        ...
        translate([0, -flap_height * height, 0]) {  // valign compensation
            scale([width, 1, 1]) {
                translate([offset_x, offset_y]) {
                    ...
                    if (is_emoji_char(letter)) {
                        text(text=letter, size=flap_height * height, font="Noto Emoji", halign="center", $fn=letter_facet_number, valign="bottom");
                    }
                    else {
